...
4. 封面:**只出提示词,不执行任何生图 skill**——用户拿提示词手动用 AI 工具生成、提供图片后,助手拷入 `assets/covers/第N集.jpg`(中文大写集数名,1920×1920 正方形 Q 版绘本风,画风锚定前集封面)。提示词经验(第 13 集踩坑确立):**不要写角色名**("小昆虫""小彩虹"这类名字会误导生图 AI),改用外观描述 + 参考图编号(参考图1=场景、2/3/4=立绘,提醒用户按编号顺序上传);构图主体控制在 1–2 个,核心动作用短动词写清("站着、踮脚、举着、盖上");加一句"全画面只有这 N 个角色"防娃娃复制/路人乱入
